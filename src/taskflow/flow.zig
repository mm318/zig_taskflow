...
pub fn init(a: std.mem.Allocator) !Flow {
    ...
    return .{
        .allocator = a,
        .tasks = std.ArrayList(*Task).init(a),
        .graph = Graph.init(a),
        .executor = ThreadPool.init(.{ .max_threads = num_cpus }),
    };
}
...
const FlowExecutionContext = struct {
    ...
    fn init(allocator: std.mem.Allocator, parent: *Flow, num_tasks: usize) !FlowExecutionContext {
        return FlowExecutionContext{
            .allocator = allocator,
            .task_contexts = try std.ArrayList(TaskExecutionContext).initCapacity(allocator, num_tasks),
            .sinks = std.ArrayList(usize).init(allocator),
...
pub fn execute(self: *Flow) !void {
    ...
    for (self.tasks.items) |task| {
        ...
        const task_context = TaskExecutionContext{
            .tp_task = ThreadPool.Task{ .callback = TaskExecutionContext.executeTaskInThreadPool, .cookie = null },
            .flow_task = task,
            .parent_context = &flow_context,
        };
